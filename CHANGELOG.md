...
- New class added `SecureWriteTransactionManager`:
...
- New methods added to `LegacySamSelectionExtension`:
    - `setStaticUnlockDataProvider(LegacySamStaticUnlockDataProviderSpi unlockDataProvider)` sets the unlock data
      provider
    ...
    - `setStaticUnlockDataProvider(LegacySamStaticUnlockDataProviderSpi unlockDataProvider, CardReader targetSamReader)`
      sets the unlock data provider and the target SAM reader for the static mode when the target SAM reader is
      initially
      available.
...
:warning: The project has been migrated from
the [Calypsonet Terminal Calypso Crypto Legacy SAM API](https://github.com/calypsonet/calypsonet-terminal-calypso-crypto-legacysam-java-api)
GitHub repository.
...
- Renamed:
    - Artifact `org.calypsonet.terminal:calypsonet-terminal-calypso-crypto-legacysam-java-api` ->
      `org.eclipse.keypop:keypop-calypso-crypto-legacysam-java-api`
